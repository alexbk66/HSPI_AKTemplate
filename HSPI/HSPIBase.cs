...
namespace HSPI_AKTemplate
{
    ...
    public abstract class HspiBase : IPlugInAPI
    {
        ...
        public bool IsConnected => HsClient.CommunicationState == CommunicationStates.Connected;
        ...
        /// <summary>
        /// Try to connect to HS.
        /// </summary>
        /// <param name="serverAddress"></param>
        /// <param name="serverPort"></param>
        /// <param name="retries"></param>
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "retries")]
        public void ConnectRetry(string serverAddress = null, int serverPort = -1, int retries = -1)
        {
            // When plugin tries to reconnect to HS - set Pause
            // So device updates are queued until connection established
            Connected(false);

            // Note - calss Options should add options for controlling retries
        ...
        /// <param name="serverAddress"></param>
        /// <param name="serverPort"></param>
        /// <returns>'true' if all good, 'false' if failed</returns>
        ...
        /// <summary>
        /// When connection with HS is established/lost - call this function to notify derived plugin
        /// </summary>
        /// <param name="isconnected"></param>
        protected abstract void Connected(bool isconnected);

        /// <summary>
        /// Main connetction function. Called from Program.cs
        /// Connects to HS and waits in infinite loop until HS calls ShutdownIO
        /// </summary>
        /// <param name="args"></param>
        /// <returns></returns>
        public int ConnectMain(string[] args)
        {
            Parser.Default.ParseArguments<Options>(args)
                .WithParsed(options => ConnectRetryMain(options));
            //Options options = new Options();
            //Parser.Default.ParseArguments(args, options);
            //ConnectRetryMain(options);
            return 0;
        }
        ...
        public void ConnectRetryMain(Options options)
        {
            // Note - calss Options should add options for controlling retries
            // If ConnectMain returned True - means proper shutdown requested (from ShutdownIO)
            // Else - disconnection issues - try to reconnect
            while (!ConnectMain(options))
            {
                Thread.Sleep(1);
            }
        }
        ...
        protected AutoResetEvent InitIOEvent = new AutoResetEvent(false);
        protected AutoResetEvent DisconnectedEvent = new AutoResetEvent(false);

        /// <summary>
        /// Connects to HS and waits in infinite loop until HS calls ShutdownIO
        /// </summary>
        /// <param name="options">Options</param>
        /// <returns>'false' if error occured - may try to reconnect</returns>
        [SuppressMessage("Microsoft.Globalization", "CA1304:SpecifyCultureInfo", MessageId = "System.String.ToLower")]
        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes")]
        [SuppressMessage("Microsoft.Design", "CA1062:Validate arguments of public methods", MessageId = "0")]
        public bool ConnectMain(Options options)
        {
            try
            {
            ...
            Console.WriteLine($"\n{Name} Connecting to HomeSeer at {options.Server}:{options.Port} ...");
            try
            {
                if (!Connect(options.Server, options.Port))
                    return false;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Connection to HomeSeer failed: {ex.Message}");
                // For timeout wait isn't necessary, it's waited already
            ...
            Console.WriteLine("Connection to HomeSeer successful. Waiting for InitIO...\n");

            if(!InitIOEvent.WaitOne(30000))
            {
                Console.WriteLine("InitIO was not called - exiting\n");
                return true;
            }

            Console.WriteLine("InitIO finished - entering main loop");

            // Wait for a shutdown signal
            //ShutdownEvent.WaitOne();
            int n = WaitHandle.WaitAny(new WaitHandle[] { DisconnectedEvent, ShutdownEvent });

            if (n == 0)
            {
                Console.WriteLine("Received DisconnectedEvent");
                return false;
            }

            Console.WriteLine("Plugin has been shut down, exiting");
            // Return 'true' indicating normal shutdown
            return true;
        }
